...
description: Use ONLY when designing how to send notifications/messages (email, push, in-app) and need to know which approach fits - direct send, queue+worker, multi-channel Observer, or Outbox for delivery guarantees. Do not use for choosing a queue library (see zai-stack-queues), for WHY notification work might live in its own service (see zai-practices-architecture-service-boundaries), or for realtime push technology like WebSockets/SSE (see zai-practices-architecture-realtime).
---
